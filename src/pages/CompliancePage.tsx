...
export default function CompliancePage() {
  ...
  const stats = useMemo(() => {
    ...
    const fullyCompliant = items.filter(
      (r) => r.soc2 && r.iso27001 && r.gdpr && r.sso && r.mfa
    ).length;
    const highRisk = items.filter((r) => r.risk === "高").length;
    const trueCount = items.reduce(
      (s, r) => s + [r.soc2, r.iso27001, r.gdpr, r.sso, r.mfa].filter(Boolean).length,
      0
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader breadcrumb={["稽核合規"]} title="稽核合規" description="檢視應用認證狀態與風險評估" />
      ...
      {isLoading ? <Skeleton className="h-64 rounded-lg" /> : <DataTable columns={columns} data={items} />}
    </div>
  );
}
